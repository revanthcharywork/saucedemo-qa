...
def test_inventory_item_count(logged_in_page):
    inv = Inventory(logged_in_page)
    assert inv.items_count() == 6 , f"Inventory items count expected to be 6, But got {inv.items_count()}"
...
@pytest.mark.inventory
def test_sort_products_high_to_low(logged_in_page):
    inv = Inventory(logged_in_page)
    inv.sort_items("hilo")
    items_data = inv.get_item_details()
    prices = []
...
@pytest.mark.inventory
def test_sort_products_z_to_a(logged_in_page):
    inv = Inventory(logged_in_page)
    inv.sort_items("za")
    items_data = inv.get_item_details()
    product_names = []
    for item in items_data:
        product_names.append(item[0])
    assert product_names == sorted(product_names, reverse=True), "Product names not sorted high to low"
